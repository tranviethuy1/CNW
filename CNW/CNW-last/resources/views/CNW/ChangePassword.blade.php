				...
				<div class="form--login">
                    <div class="dangNhap">
						...
						<form action="{!! route('changePassword') !!}" method="POST">
							<input type="hidden" name="_token" value="{{ csrf_token() }}">
	  						<div class="form-group">
	    						...
	    						<input type="email" class="form-control" id="exampleInputEmail1" aria-describedby="emailHelp" placeholder="Enter email" name="email_logined">
	  						</div>
	  						<div class="form-group">
								<label for="exampleInputEmail1"><i class="fas fa-key"></i>Current password</label>
	    						<input type="password" class="form-control" id="exampleInputPassword1" placeholder="Password" name="current_password_login">
	  						</div>
	  						<div class="form-group">
								<label for="exampleInputEmail1"><i class="fas fa-key"></i>New password</label>
	    						<input type="password" class="form-control" id="exampleInputPassword1" placeholder="Password" name="new_password_login">
	  						</div>	  
	  						<button type="submit" class="btn btn-primary">Submit</button>
	  						<div class="status">
	  							@if(session('status_change_password'))
		  						<span class="alert alert-danger form-control" style="margin-top: 38px;border-radius: 56px;">
		  							{!! session('status_change_password') !!}
		  						</span>
		  						@endif
	                            @if(session('status_change_password_failed'))
		  						<span class="alert alert-danger form-control" style="margin-top: 38px;border-radius: 56px;">
		  							{!! session('status_change_password_failed') !!}
		  						</span>
		  						@endif
	  						</div>
						</form>
						<div class="login--signup">
							<span>
								<a href="{!! url('home/login')!!}">Back Login</a>
							</span>
						</div>
					</div>		
				</div>
